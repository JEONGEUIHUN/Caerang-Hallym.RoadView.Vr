...
● 한림대학교의 길과 구역에 이름이 있다는 것을 모르시는 분들을 위해 자신의 현재위치를 화면에 보여줌으로써 알게 될 것 입니다.
● 또한 지도 선택으로 가고 싶은 곳을 선택하여 이동할 수 있습니다.
● NPC를 구현해서 해당 건물의 설명 및 동영상 시청이 가능합니다.
● 신입생은 물론 재학생까지 가보지 못한 길, 예상치 못한 곳에 길이 있는 것 등 알려드리기 위해 구현했습니다. 
● VR장비가 없으신 분들은 Web서버 주소를 통해 Unity 프로젝트를 올려서 누구나 접속이 가능 하도록 구현했습니다.
